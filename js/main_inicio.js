(function(){ 
	...
	*/
	...
	const validarCaracteresDeNotas = (input, valor, form, limitsCaract) =>{

		//caracteres NO disponibles en las entradas de los datos de las notas
		...
		for(let i=0; i<valor.length; i++){
			...
			if (valor[i] == '#' || valor[i] == '$' || valor[i] == '>' || valor[i] == '<' || valor[i] == '/'){
				...
				error(input, true, form, `${valor[i]} caracter Invalido!`);

				break;
			
			} else{

				error(input, false, form, '');
			}

				//cantidad de caracteres
			if(valor.length > limitsCaract){

				error(input, true, form, `Limites de caracteres ${limitsCaract}`);
		...
		form.addEventListener('submit', (event)=>{
			...
			*/
			...
			const datos_de_nota = {
				...
				nombre : event.target.nombre_agg.value,
				contenido : event.target.contenido_agg.value,
				color : event.target.color_agg.value,
		...
		countRequest_CN.onsuccess = () =>{
			...
			objectStore.openCursor().onsuccess = (e) =>{
				...
				nota.innerHTML = //no existen notas
					...
					<div class="nota" style="background: #e8c129bf;" id="NC">
							<div>
								<div class="nombre-notas">
				...
				html : `
					<form method="get" id="form_agg">
						...
						<div>
							...
							<div>
								<select class="input-form" name="color_agg" id="color_nota_agg">
